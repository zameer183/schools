...
type StudentResultsPageData = {
  student: { id: string; classId: string | null; admissionNo: string };
  results: ResultItem[];
};
  ...
  return (
    <Card className="p-8">
      <div className="flex flex-col items-center py-6 text-center">
        ...
        <p className="mt-2 text-sm text-[#6B7280]">Unable to load your exam results right now. Please refresh once the connection recovers.</p>
      </div>
    </Card>
  );
}
...
const getCachedStudentResultsData = unstable_cache(
  async (userId: string) => {
    ...
    return { student, results } satisfies StudentResultsPageData;
  },
...
export default async function StudentResultsPage() {
  ...
  let dashboardData: Awaited<ReturnType<typeof getCachedStudentResultsData>> | null = null;
  try {
    dashboardData = await getCachedStudentResultsData(session.id);
  } catch (error) {
  ...
  const { student, results } = dashboardData;
