      ...
      {selectedClass ? (
        <div className="rounded-2xl bg-white p-4 shadow-[0_4px_12px_rgba(0,0,0,0.08)] sm:p-6">
          <div className="mb-3 flex justify-end gap-2 print:hidden">
            ...
            <a
              href={`/api/reports/export?type=finance-class&classId=${selectedClass.id}&month=${monthKey}`}
              download
          ...
          <div className="grid gap-3 sm:grid-cols-4">
            <SummaryBox label="Class" value={`${selectedClass.name} ${selectedClass.section}`} tone="neutral" />
            <SummaryBox label="Month" value={label} tone="neutral" />
            <SummaryBox label="Paid Students" value={String(paidStudents)} tone="green" />
            <SummaryBox label="Unpaid Students" value={String(unpaidStudents)} tone="red" />
          </div>
          ...
          <div className="mt-4 overflow-x-auto rounded-xl border border-[#e5e7eb]">
            <table className="min-w-[760px] w-full border-collapse text-sm">
              ...
              <tbody>
                {rows.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-3 py-8 text-center text-sm text-[#6b7280]">No students found for this class/month.</td>
                  </tr>
                ) : (
                  rows.map((row) => (
              ...
              <tfoot>
                <tr className="bg-[#f8fafc]">
                  <td className="border-r border-[#e5e7eb] px-3 py-2 font-bold">Class Total</td>
...
function SummaryBox({ label, value, tone }: { label: string; value: string; tone: 'neutral' | 'green' | 'red' }) {
  const tones = {
